...
class KeyboardLayout: NSObject, KeyboardKeyProtocol {
    ...
    func updateKeyCap(_ key: KeyboardKey, model: Key, fullReset: Bool, uppercase: Bool, characterUppercase: Bool, shiftState: ShiftState) {
        if fullReset {
            ...
            case Key.KeyType.placeholder:
                if key.shape == nil {
                    let placeholder = self.getShape(PlaceholderShape.self)
                    key.shape = placeholder
                }
            default:
                break
            }
    ...
    func generateKeyFrames(_ model: Keyboard, bounds: CGRect, page pageToLayout: Int) -> [Key:CGRect]? {
        ...
        for (p, page) in model.pages.enumerated() {
            ...
            let letterKeyWidth: CGFloat = {
                let totalGaps = (sideEdges * CGFloat(2)) + (keyGap * CGFloat(mostKeysInRow - 1)) - 160
            ...
            if(true) {
                ...
                for (r, row) in page.rows.enumerated() {
                    ...
                    frames = layoutKeysRow(pageNum: p, rowNum: r, row: row, keyWidth: 28, sideSpace: 4, keyGap: 7, frame: frame)
                ...
                for (r, row) in page.rows.enumerated() {
                    ...
                    if self.characterRowHeuristic(row) {
                        frames = self.layoutCharacterWithPlaceholderRow(row, keyWidth: letterKeyWidth, gapWidth: 7, frame: frame)
                    }
                        
                        // character row with side buttons: shift, backspace, etc.
                    else if self.doubleSidedRowHeuristic(row) {
                        frames = self.layoutCharacterWithSidesWithPlaceholderRow(row, frame: frame, isLandscape: isLandscape, keyWidth: letterKeyWidth, keyGap: keyGap)
                    }
    ...
    func characterRowHeuristic(_ row: [Key]) -> Bool {
        return (row.count >= 1 && (row[0].isCharacter || row[0].isGap))
    }
    
    func doubleSidedRowHeuristic(_ row: [Key]) -> Bool {
        return (row.count >= 3 && !row[0].isCharacter && row[1].isCharacter)
    }
    func layoutCharacterWithPlaceholderRowLeft(_ row: [Key], keyWidth: CGFloat, gapWidth: CGFloat, frame: CGRect) -> [CGRect]  {
        var frames = [CGRect]()
    ...
    func layoutCharacterWithPlaceholderRowRight(_ row: [Key], keyWidth: CGFloat, gapWidth: CGFloat, frame: CGRect) -> [CGRect]  {
        var frames = [CGRect]()
        
        
        return frames;
    }
    func layoutCharacterWithPlaceholderRow(_ row: [Key], keyWidth: CGFloat, gapWidth: CGFloat, frame: CGRect) -> [CGRect]  {
        var frames = [CGRect]()
        var actKeyNum = 0
        var gapKeyNum = 0
        for (_, key) in row.enumerated(){
            if(key.type == Key.KeyType.placeholder) {
                
            }
            else if(key.type == Key.KeyType.gap){
                gapKeyNum = gapKeyNum + 1
            }
            else {
                actKeyNum  = actKeyNum + 1
            }
        }
        
        
        
        let placehoderSpace = CGFloat(220)//frame.width - (CGFloat(actKeyNum) * keyWidth + CGFloat(row.count - 1) * gapWidth + CGFloat(gapKeyNum)*16) - CGFloat(8)*2
        
        
        let keySpace = CGFloat(row.count-1) * keyWidth + CGFloat(row.count - 1) * gapWidth + 160
        var actualGapWidth = gapWidth

        var sideSpace = CGFloat(4)//(frame.width - keySpace) / CGFloat(2)
        if sideSpace < 0 {
            sideSpace = 0
        ...
        for (index, key) in row.enumerated() {
            ...
            if roundedOrigin + keyWidth > frame.origin.x + frame.width {
                rect  = CGRect(x: rounded(frame.origin.x + frame.width - keyWidth), y: frame.origin.y, width: width, height: height)
                //                frames.append()
            }
            else {
                rect  = CGRect(x: rounded(currentOrigin), y: frame.origin.y, width: width, height: height)
                //                frames.append()
            }
            
            if(index == row.count-1) {
                if(rect.origin.x + rect.size.width < frame.size.width - sideSpace) {
                    width = frame.size.width - sideSpace - rect.origin.x
                    rect = CGRect(x: rounded(currentOrigin), y: frame.origin.y, width: width, height: height)
                }
            }
            ...
            currentOrigin += (width + actualGapWidth)
        }
        return frames;
    }
    ...
    func layoutCharacterRow(_ row: [Key], keyWidth: CGFloat, gapWidth: CGFloat, frame: CGRect) -> [CGRect] {
        ...
        let keySpace = CGFloat(row.count) * keyWidth + CGFloat(row.count - 1) * gapWidth + 200
        var actualGapWidth = gapWidth
...
//                frames.append()
    ...
    func layoutCharacterWithSidesWithPlaceholderRow(_ row: [Key], frame: CGRect, isLandscape: Bool, keyWidth: CGFloat, keyGap: CGFloat) -> [CGRect] {
        var frames = [CGRect]()
        var actKeyNum = 0
        var gapKeyNum = 0
        for (_, key) in row.enumerated(){
        ...
        let placehoderSpace = CGFloat(220)
        var sideSpace = CGFloat(4)
        var currentOrigin = frame.origin.x + sideSpace
...
//        assert(keysAfterSpace == 1, "invalid number of keys after space (only default 1 currently supported)")
        ...
        var spaceWidth = CGFloat(100);//frame.width - leftSideAreaWidth - rightSideAreaWidth - gapWidth * CGFloat(2)
        spaceWidth = rounded(spaceWidth)
        
        var currentOrigin = frame.origin.x
        var beforeSpace: Bool = true
        for (k, key) in row.enumerated() {
            var actHeight = frame.height
            if key.type == Key.KeyType.placeholder {
                actHeight = 0
            }
            if key.type == Key.KeyType.space {
                ...
                frames.append(CGRect(x: rounded(currentOrigin), y: frame.origin.y, width: spaceWidth, height: actHeight))
                currentOrigin += (spaceWidth + gapWidth)
                beforeSpace = false
            }
            else if key.type == Key.KeyType.placeholder {
                frames.append(CGRect(x: rounded(currentOrigin), y: frame.origin.y, width: 100, height: actHeight))
                currentOrigin += (100 + gapWidth)
            }
            else if beforeSpace {
                if hasButtonInMicButtonPosition && k == 2 { //mic button position
                    frames.append(CGRect(x: rounded(currentOrigin), y: frame.origin.y, width: micButtonWidth, height: actHeight))
                    currentOrigin += (micButtonWidth + gapWidth)
                }
                else {
                    frames.append(CGRect(x: rounded(currentOrigin), y: frame.origin.y, width: leftButtonWidth, height: actHeight))
                    currentOrigin += (leftButtonWidth + gapWidth)
                }
            }
            else {
                frames.append(CGRect(x: rounded(currentOrigin), y: frame.origin.y, width: rightButtonWidth, height: actHeight))
